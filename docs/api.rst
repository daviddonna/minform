...
~~~~~~~~~~~~~~~~~~
    ...
    * A :meth:`~BinaryItem.pack` method. The type of :paramref:`~minform.BinaryItem.pack.data` should be compatible with the type returned by the :meth:`
      ~BinaryItem.unpack` method (below).

    * An :meth:`~BinaryItem.unpack` method. You can expect :paramref:`
      ~minform.BinaryItem.unpack.buf` to have ``self.size`` bytes when the method is
